...
import { useOpportunity, useOpportunities } from '@/lib/hooks/use-opportunities';
import type { OppApplication } from '@/lib/hooks/use-opportunities';
...
function ApplicationRow({ app }: { app: OppApplication }) {
  return (
...
export default function OpportunityDetailPage() {
  ...
  const { data: opp, isLoading } = useOpportunity(params.id);
  const { data: allOpps = [] } = useOpportunities();

  const similar = allOpps.filter((o) => o.id !== params.id && o.type === opp?.type).slice(0, 3);
  ...
  if (!opp) {
    return (
      <div className="text-center py-16">
        <p className="text-muted-foreground text-sm">Opportunity not found.</p>
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <PageHeader
        title={opp.title}
        description={`${opp.projectName} · ${opp.organizationName}`}
        breadcrumb={
          <>
            <Link href="/dashboard/opportunities" className="hover:text-foreground flex items-center gap-1">
              <ArrowLeft className="h-3 w-3" />
              Opportunities
            </Link>
            <ChevronRight className="h-3 w-3" />
            <span className="text-foreground truncate max-w-[200px]">{opp.title}</span>
          </>
        }
        actions={
          <>
            ...
            <Button
              size="sm"
              variant={opp.status === 'open' ? 'outline' : 'default'}
              className={cn(opp.status === 'open' && 'text-destructive border-destructive/30 hover:bg-destructive/10')}
              onClick={() => toast({ title: opp.status === 'open' ? 'Opportunity closed' : 'Opportunity reopened' })}
            >
      ...
      {/* Metrics */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <MetricCard title="Applications" value={opp.applicantCount} icon={Users} accent />
        <MetricCard title="Views" value={opp.viewCount} icon={Eye} />
        <MetricCard title="Days Open" value={opp.daysOpen} icon={Clock} />
        <MetricCard title="Difficulty" value={opp.difficulty} icon={BarChart2} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5">
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="flex flex-wrap items-center gap-2 mb-4">
              <Badge variant={TYPE_BADGE_VARIANT[opp.type] ?? 'muted'} className="text-xs capitalize">
                {opp.type.replace(/-/g, ' ')}
              </Badge>
              <span className={cn(
              ...
              {opp.bountyAmount && (
                <Badge variant="warning" className="text-xs">
                  {opp.bountyAmount} {opp.bountyToken} bounty
                </Badge>
              )}
            </div>
              ...
              <div className="px-5 pt-4 border-b border-border">
                <TabsList>
                  <TabsTrigger value="applications">
                    Applications
                    <Badge variant="muted" className="ml-1.5 text-[10px] px-1">{opp.applications.length}</Badge>
                  </TabsTrigger>
                </TabsList>
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="flex items-center gap-3">
              <div className="h-9 w-9 rounded-lg bg-muted flex items-center justify-center text-sm font-bold text-muted-foreground shrink-0">
                {opp.projectName.charAt(0)}
              </div>
              <div>
                <p className="text-sm font-medium text-foreground">{opp.projectName}</p>
            ...
            <Button variant="outline" size="sm" className="w-full mt-3 text-xs h-7 gap-1.5" asChild>
              <Link href={`/dashboard/projects/${opp.projectId}`}>
          ...
          {similar.length > 0 && (
            <div className="rounded-xl border border-border bg-card p-4">
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Similar Opportunities</h3>
