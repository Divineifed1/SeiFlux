...
export default function AdminPage() {
  ...
  const apiPending = (projectsData as { id: string; name: string; organizationId?: string; status: string; tags?: string[]; description?: string }[])
    .filter((p) => p.status === 'pending')
    .map<PendingProject>((p) => ({
      id: p.id,
      name: p.name,
  ...
  const pendingProjects = [...apiPending, ...submittedPending];

  const [dismissedReports, setDismissedReports] = React.useState<Set<string>>(new Set());
  const reports: ReportedIssue[] = [];
  const visibleReports = reports.filter((r) => !dismissedReports.has(r.id));

  const [users, setUsers] = React.useState<AdminUser[]>([]);
  React.useEffect(() => {
    if (Array.isArray(usersData)) {
      setUsers(
        (usersData as { id: string; name: string; handle?: string; role: string; joinedAt?: string; status?: string }[]).map((u) => ({
          id: u.id,
          name: u.name,
          handle: u.handle ?? u.name.toLowerCase().replace(/\s+/g, '_'),
          role: u.role === 'maintainer' ? 'maintainer' : 'contributor',
  ...
  React.useEffect(() => {
    if (Array.isArray(orgsData)) {
      setOrgs(
        (orgsData as { id: string; name: string; slug: string; projectCount?: number; memberCount?: number; status?: string; createdAt?: string }[]).map((o) => ({
          id: o.id,
          name: o.name,
          slug: o.slug,
          projects: o.projectCount ?? 0,
          members: o.memberCount ?? 0,
          status: o.status === 'suspended' ? 'suspended' : 'active',
          joinedAt: o.createdAt ? new Date(o.createdAt).toLocaleDateString() : '—',
        }))
      );
    }
  }, [orgsData]);
  ...
  const platformMetrics = [
    { title: 'Total Users', value: usersData.length, icon: Users, accent: true },
    { title: 'Total Projects', value: projectsData.length, icon: FolderKanban },
    { title: 'Active Contributors', value: usersData.filter((u: { role?: string }) => u.role === 'contributor').length, icon: UserCheck },
    { title: 'Active Maintainers', value: usersData.filter((u: { role?: string }) => u.role === 'maintainer').length, icon: ShieldCheck },
    { title: 'Open Issues', value: analytics?.repoActivity.reduce((acc: number, r) => acc + (r.issues ?? 0), 0) ?? 0, icon: FileText },
    { title: 'Organizations', value: orgsData.length, icon: Building2 },
  ];

  const [previewProject, setPreviewProject] = React.useState<PendingProject | null>(null);
  ...
  const dismissReport = (id: string) => {
    setDismissedReports((prev) => new Set(prev).add(id));
      ...
      <Tabs defaultValue="projects">
          ...
          <TabsTrigger value="reports">
            Reports
             {visibleReports.length > 0 && (
               <Badge variant="destructive" className="ml-1.5 text-[10px] px-1">{visibleReports.length}</Badge>
             )}
          </TabsTrigger>
        ...
        <TabsContent value="analytics" className="mt-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="rounded-lg border border-border bg-card p-5">
              <SectionHeader title="Platform Health (7d)" className="mb-4" />
              <div className="space-y-3">
                {[
                  { label: 'Active Contributors', value: analytics.activeContributors, max: Math.max(analytics.activeContributors, 1) * 2 },
                  { label: 'Open PRs', value: analytics.openPRs, max: Math.max(analytics.openPRs, 1) * 2 },
                  { label: 'Merged PRs', value: analytics.mergedPRs, max: Math.max(analytics.mergedPRs, 1) * 2 },
                  { label: 'Issue Resolution %', value: analytics.issueResolutionRate, max: 100 },
                ].map((stat) => (
                  <div key={stat.label}>
            ...
            <div className="rounded-lg border border-border bg-card p-5">
              ...
              <div className="space-y-2.5">
                {analytics.issueCategories.map((item) => (
                  <div key={item.category} className="flex items-center gap-3">
                    <span className="text-xs text-muted-foreground w-24 shrink-0">{item.category}</span>
                    <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
                      <div className="h-full rounded-full bg-primary/50" style={{ width: `${item.resolutionRate}%` }} />
                    </div>
            ...
            <div className="rounded-lg border border-border bg-card p-5 lg:col-span-2">
              <SectionHeader title="Repository Activity" className="mb-4" />
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border text-xs text-muted-foreground">
                      ...
                      <th className="text-right py-2 font-medium">PRs Opened</th>
                      <th className="text-right py-2 font-medium">PRs Merged</th>
                      <th className="text-right py-2 font-medium">Issues</th>
                    </tr>
                  </thead>
                  <tbody>
                    {analytics.repoActivity.map((repo) => (
                      <tr key={repo.repo} className="border-b border-border/50 last:border-0">
                        <td className="py-2.5 text-foreground">{repo.repo}</td>
                        <td className="py-2.5 text-right tabular-nums text-muted-foreground">{repo.commits}</td>
                        <td className="py-2.5 text-right tabular-nums text-muted-foreground">{repo.prsOpened}</td>
                        <td className="py-2.5 text-right tabular-nums text-muted-foreground">{repo.prsMerged}</td>
                        <td className="py-2.5 text-right tabular-nums text-muted-foreground">{repo.issues}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
